module private FsNativeAutoComplete.Tests.CodeFixTests.HelpersTests
// `src\FsNativeAutoComplete\CodeFixes.fs` -> `FsNativeAutoComplete.CodeFix`
...
let makeText (textFactory: FsNativeAutoComplete.ISourceTextFactory) (text: string) =
...
let tests textFactory =
  testList ($"{nameof (FsNativeAutoComplete)}.{nameof (FsNativeAutoComplete.CodeFix)}") [ navigationTests textFactory ]
