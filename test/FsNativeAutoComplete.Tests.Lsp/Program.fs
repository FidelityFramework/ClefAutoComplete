...
open Serilog.Core
open Serilog.Events
open FsNativeAutoComplete.Tests
open FsNativeAutoComplete.Tests.CoreTest
open FsNativeAutoComplete.Tests.ScriptTest
open FsNativeAutoComplete.Tests.ExtensionsTests
open FsNativeAutoComplete.Tests.InteractiveDirectivesTests
open FsNativeAutoComplete.Tests.Lsp.CoreUtilsTests
open FsNativeAutoComplete.Tests.CallHierarchy
...
let loaders =
  ...
  | Some(EqIC "WorkspaceLoader") ->
    [ "Ionide WorkspaceLoader",
      (fun toolpath -> WorkspaceLoader.Create(toolpath, FsNativeAutoComplete.Core.ProjectLoader.globalProperties)) ]
  | Some(EqIC "ProjectGraph") ->
    [ "MSBuild Project Graph WorkspaceLoader",
      (fun toolpath ->
        WorkspaceLoaderViaProjectGraph.Create(toolpath, FsNativeAutoComplete.Core.ProjectLoader.globalProperties)) ]
  | _ ->
    [ "Ionide WorkspaceLoader",
      (fun toolpath -> WorkspaceLoader.Create(toolpath, FsNativeAutoComplete.Core.ProjectLoader.globalProperties))
      // "MSBuild Project Graph WorkspaceLoader", (fun toolpath -> WorkspaceLoaderViaProjectGraph.Create(toolpath, FsNativeAutoComplete.Core.ProjectLoader.globalProperties))
      ]
...
[<EntryPoint>]
let main args =
  let serviceName = "FsNativeAutoComplete.Tests.Lsp"

  use traceProvider =
    let version = FsNativeAutoComplete.Utils.Version.info().Version

    Sdk
      .CreateTracerProviderBuilder()
      .AddSource(FsNativeAutoComplete.Utils.Tracing.serviceName, Tracing.fscServiceName, serviceName)
      .SetResourceBuilder(
  ...
  // let trace = traceProvider.GetTracer("FsNativeAutoComplete.Tests.Lsp")
