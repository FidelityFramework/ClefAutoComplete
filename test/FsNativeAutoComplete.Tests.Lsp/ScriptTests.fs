module FsNativeAutoComplete.Tests.ScriptTest
    ...
    async {
      ...
      let _dependencyManagerAssemblyDir =
        Path.Combine(
          ...
          "..",
          "FsNativeAutoComplete.DependencyManager.Dummy",
          "bin",
          "Debug",
          "netstandard2.0"
        )
...
let scriptProjectOptionsCacheTests state =
  let server =
    async {
      ...
      match server with
      | :? FsNativeAutoComplete.Lsp.AdaptiveFSharpLspServer as server -> server.ScriptFileProjectOptions.Add(options.Add)
