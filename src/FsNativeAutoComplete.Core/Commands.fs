namespace FsNativeAutoComplete
...
open FsNativeAutoComplete.RecordStubGenerator
open Utils
...
type Commands() =
  ...
  static member InlayHints(text, tyRes: ParseAndCheckResults, range, ?showTypeHints, ?showParameterHints) =
    ...
    FsNativeAutoComplete.Core.InlayHints.provideHints (text, tyRes, range, hintConfig)
