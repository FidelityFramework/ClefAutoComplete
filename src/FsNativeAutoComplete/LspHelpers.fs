module FsNativeAutoComplete.LspHelpers
...
module Conversions =
  ...
  let findDeclToLspLocation (decl: FsNativeAutoComplete.FindDeclarationResult) : LspLocation =
    match decl with
    | FsNativeAutoComplete.FindDeclarationResult.ExternalDeclaration ex ->
      let fileUri = Path.FilePathToUri ex.File

      { Uri = fileUri
        Range = fcsPosToProtocolRange ex.Position }
    | FsNativeAutoComplete.FindDeclarationResult.Range r -> fcsRangeToLspLocation r
    | FsNativeAutoComplete.FindDeclarationResult.File file ->
...
module Workspace =
  ...
  open FsNativeAutoComplete.CommandResponse
