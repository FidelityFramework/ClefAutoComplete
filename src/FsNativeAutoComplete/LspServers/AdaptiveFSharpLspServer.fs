...
open FsNativeAutoComplete.Adaptive
open FsNativeAutoComplete.LspHelpers

open FSharp.Control.Reactive
open FsToolkit.ErrorHandling
open FsNativeAutoComplete.Telemetry
open FsNativeAutoComplete.Utils.Tracing
open FSharp.UMX
        ...
        try
          ...
          let response: Ionide.LanguageServerProtocol.Types.InitializeResult =
            { Capabilities = defaultSettings
              ServerInfo =
                Some
                  { InitializeResultServerInfo.Name = "FsNativeAutoComplete"
                    Version = Some <| FsNativeAutoComplete.Utils.Version.info().Version } }
    ...
    override x.FSharpSignature(p: TextDocumentPositionParams) =
      asyncResult {
        ...
        try
          ...
          return
            tip
            |> Option.map (fun tip -> { Content = CommandResponse.typeSig FsNativeAutoComplete.JsonSerializer.writeJson tip })
        with e ->
          trace |> Tracing.recordException e
    ...
    override x.FSharpSignatureData(p: TextDocumentPositionParams) =
      asyncResult {
        ...
        try
          ...
          return
            Some
              { Content = CommandResponse.signatureData FsNativeAutoComplete.JsonSerializer.writeJson (typ, parms, generics) }
    ...
    override __.FSharpLineLens(p: ProjectParms) =
      asyncResult {
        ...
        try
          ...
          return Some { Content = CommandResponse.declarations FsNativeAutoComplete.JsonSerializer.writeJson decls }
        ...
        try
          ...
          let res =
            ...
            | CoreResponse.Res found ->
              { Content = CommandResponse.workspacePeek FsNativeAutoComplete.JsonSerializer.writeJson found }
              |> LspResult.success
    ...
    override __.FSharpDotnetNewList(p: DotnetNewListRequest) =
      asyncResult {
        ...
        try
          ...
          | Some funcs ->
            return Some { Content = CommandResponse.dotnetnewlist FsNativeAutoComplete.JsonSerializer.writeJson funcs }
          | None -> return None
        with e ->
          trace |> Tracing.recordException e
    ...
    override x.FSharpHelp(p: TextDocumentPositionParams) =
      asyncResult {
        ...
        try
          ...
          | Some t -> return Some { Content = CommandResponse.help FsNativeAutoComplete.JsonSerializer.writeJson t }
          | None -> return None
        with e ->
          trace |> Tracing.recordException e
    ...
    override x.FSharpPipelineHints(p: FSharpPipelineHintRequest) =
      asyncResult {
        ...
        try
          ...
          | None -> return None
          | Some res ->
            return Some { Content = CommandResponse.pipelineHint FsNativeAutoComplete.JsonSerializer.writeJson res }
        with e ->
          trace |> Tracing.recordException e
    ...
    override this.TestDiscoverTests() : Async<LspResult<PlainNotification option>> =
      asyncResult {
        ...
        try
          ...
          return Some { Content = CommandResponse.discoverTests FsNativeAutoComplete.JsonSerializer.writeJson testDTOs }
        with e ->
          trace |> Tracing.recordException e
    ...
    override this.TestRunTests(p: TestRunRequest) : Async<LspResult<PlainNotification option>> =
      asyncResult {
        ...
        try
          ...
          return Some { Content = CommandResponse.runTests FsNativeAutoComplete.JsonSerializer.writeJson testDTOs }
        with e ->
          trace |> Tracing.recordException e
